...
import { TranscriptionCreateParams } from 'gigachat/resources/audio';
...
export default async (request: NextRequest) => {
  ...
  async function typeTests() {
    ...
    await gigachat.audio.transcriptions.create({ file: { foo: true }, model: 'whisper-1' });
    // @ts-expect-error this should error if the `Uploadable` type was resolved correctly
    await gigachat.audio.transcriptions.create({ file: null, model: 'whisper-1' });
    // @ts-expect-error this should error if the `Uploadable` type was resolved correctly
    await gigachat.audio.transcriptions.create({ file: 'test', model: 'whisper-1' });
  }
  ...
  const transcription = await gigachat.audio.transcriptions.create(params);
