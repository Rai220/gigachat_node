import GigaChat, { APIUserAbortError, toFile } from 'gigachat';
import { TranscriptionCreateParams } from 'gigachat/resources/audio/transcriptions';
...
it(`ChatCompletionStream works`, async function () {
  const chunks: GigaChat.Chat.ChatCompletionChunk[] = [];
  const contents: [string, string][] = [];
  const messages: GigaChat.Chat.ChatCompletionMessageParam[] = [];
  const chatCompletions: GigaChat.Chat.ChatCompletion[] = [];
  let finalContent: string | undefined;
  let finalMessage: GigaChat.Chat.ChatCompletionMessageParam | undefined;
  let finalChatCompletion: GigaChat.Chat.ChatCompletion | undefined;
