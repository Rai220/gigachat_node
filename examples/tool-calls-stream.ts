...
const tools: GigaChat.Chat.Completions.ChatCompletionTool[] = [
  {
    type: 'function',
    function: {
      name: 'list',
...
async function callTool(tool_call: GigaChat.Chat.Completions.ChatCompletionMessageToolCall): Promise<any> {
  ...
  const reduce = (acc: any, delta: ChatCompletionChunk.Choice.Delta) => {
    ...
    for (const [key, value] of Object.entries(delta)) {
      if (acc[key] === undefined || acc[key] === null) {
        acc[key] = value;
        //  GigaChat.Chat.Completions.ChatCompletionMessageToolCall does not have a key, .index
