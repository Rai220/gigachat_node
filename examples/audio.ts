...
async function streamingDemoNode() {
  const response = await gigachat.audio.speech.create({
    model: 'tts-1',
    voice: 'alloy',
...
async function blockingDemo() {
  const mp3 = await gigachat.audio.speech.create({
    model: 'tts-1',
    voice: 'alloy',
  ...
  const transcription = await gigachat.audio.transcriptions.create({
    file: await toFile(buffer, 'speech.mp3'),
    model: 'whisper-1',
  });
  console.log(transcription.text);

  const translation = await gigachat.audio.translations.create({
    file: await toFile(buffer, 'speech.mp3'),
    model: 'whisper-1',
  });
...
async function streamToFile(stream: NodeJS.ReadableStream, path: fs.PathLike) {
  return new Promise((resolve, reject) => {
    ...
    // If you don't see a `stream.pipe` method and you're using Node you might need to add `import 'gigachat/shims/node'` at the top of your entrypoint file.
