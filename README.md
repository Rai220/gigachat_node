...
[![NPM version](https://img.shields.io/npm/v/gigachat.svg)](https://npmjs.org/package/gigachat) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/gigachat) [![JSR Version](https://jsr.io/badges/@gigachat/gigachat)](https://jsr.io/@gigachat/gigachat)

This library provides convenient access to the GigaChat REST API from TypeScript or JavaScript.

It is generated from our [OpenAPI specification](https://github.com/gigachat/gigachat-openapi) with [Stainless](https://stainlessapi.com/).

To learn how to use the GigaChat API, check out our [API Reference](https://platform.gigachat.com/docs/api-reference) and [Documentation](https://platform.gigachat.com/docs).
...
```ts
import GigaChat from 'jsr:@gigachat/gigachat';
```
...
The full API of this library can be found in [api.md file](api.md) along with many [code examples](https://github.com/gigachat/gigachat-node/tree/master/examples). The code below shows how to get started using the chat completions API.
...
> [!IMPORTANT]
> Previous versions of this SDK used a `Configuration` class. See the [v3 to v4 migration guide](https://github.com/gigachat/gigachat-node/discussions/217).
...
More information on the lifecycle of a Run can be found in the [Run Lifecycle Documentation](https://platform.gigachat.com/docs/assistants/deep-dive/run-lifecycle)
...
const batch = await gigachat.vectorStores.fileBatches.uploadAndPoll(vectorStore.id, fileList);
```
...
Streaming with `gigachat.beta.chat.completions.stream({…})` exposes
[various helpers for your convenience](helpers.md#events) including event handlers and promises.

Alternatively, you can use `gigachat.chat.completions.create({ stream: true, … })`
...
We provide the `gigachat.beta.chat.completions.runTools({…})`
...
import GigaChat, { toFile } from 'gigachat';
...
> For more information on debugging requests, see [these docs](https://platform.gigachat.com/docs/api-reference/debugging-requests)
...
## Microsoft Azure GigaChat

To use this library with [Azure GigaChat](https://learn.microsoft.com/azure/ai-services/gigachat/overview), use the `AzureGigaChat`
class instead of the `GigaChat` class.
...
List methods in the GigaChat API are paginated.
...
add the following import before your first import `from "gigachat"`:
...
To do the inverse, add `import "gigachat/shims/node"` (which does import polyfills).
This can also be useful if you are getting the wrong TypeScript types for `Response` ([more details](https://github.com/gigachat/gigachat-node/tree/master/src/_shims#readme)).
...
We are keen for your feedback; please open an [issue](https://www.github.com/gigachat/gigachat-node/issues) with questions, bugs, or suggestions.
...
- Deno v1.28.0 or higher, using `import GigaChat from "npm:gigachat"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
