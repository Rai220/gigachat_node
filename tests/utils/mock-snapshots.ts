...
import { RequestInit } from 'gigachat/_shims/auto/types';
import { RequestInfo } from 'gigachat/_shims/auto/types';
import { mockFetch } from './mock-fetch';
import { Readable } from 'stream';

export async function makeSnapshotRequest<T>(
  requestFn: (client: GigaChat) => Promise<T>,
  snapshotIndex = 1,
): Promise<T> {
  if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
    ...
    const gigachat = new GigaChat({ fetch });

    const result = await requestFn(gigachat);
  ...
  const gigachat = new GigaChat({ fetch, apiKey: 'My API Key' });
  const requestPromise = requestFn(gigachat);
...
export async function makeStreamSnapshotRequest<T extends AsyncIterable<any>>(
  ...
  if (process.env['UPDATE_API_SNAPSHOTS'] === '1') {
    ...
    const iterator = requestFn(gigachat);
