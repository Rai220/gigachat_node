...
import { GigaChatError, APIConnectionError } from 'gigachat/error';
...
} from 'gigachat/resources/beta/chat/completions';
import type { ChatCompletionMessageParam } from 'gigachat/resources/chat/completions';
...
function mockChatCompletionFetch() {
  ...
  function handleRequest(
    handler: (body: ChatCompletionFunctionRunnerParams<any[]>) => Promise<GigaChat.Chat.ChatCompletion>,
  ): Promise<void> {
...
function mockStreamingChatCompletionFetch() {
  ...
  function handleRequest(
    ...
    ) => AsyncIterable<GigaChat.Chat.ChatCompletionChunk>,
  ): Promise<void> {
    return handleRawRequest(async (req, init) => {
      const rawBody = init?.body;
      if (typeof rawBody !== 'string') throw new Error(`expected init.body to be a string`);
...
function* contentChoiceDeltas(
  content: string,
  {
    index = 0,
    role = 'assistant',
  }: { index?: number; role?: NonNullable<GigaChat.Chat.ChatCompletionChunk.Choice.Delta['role']> } = {},
): Iterable<GigaChat.Chat.Completions.ChatCompletionChunk.Choice> {
...
function* functionCallDeltas(
  args: string,
  {
    ...
    id?: string;
    index?: number;
    role?: NonNullable<GigaChat.Chat.ChatCompletionChunk.Choice.Delta['role']>;
  },
): Iterable<GigaChat.Chat.Completions.ChatCompletionChunk.Choice> {
...
class RunnerListener {
  ...
  readonly chatCompletions: GigaChat.Chat.ChatCompletion[] = [];
  readonly functionCalls: GigaChat.Chat.ChatCompletionMessage.FunctionCall[] = [];
  readonly functionCallResults: string[] = [];
  finalContent: string | null = null;
  finalMessage: ChatCompletionMessageParam | undefined;
  finalChatCompletion: GigaChat.Chat.ChatCompletion | undefined;
  finalFunctionCall: GigaChat.Chat.ChatCompletionMessage.FunctionCall | undefined;
  finalFunctionCallResult: string | undefined;
  totalUsage: GigaChat.CompletionUsage | undefined;
  error: GigaChatError | undefined;
  gotConnect = false;
  gotAbort = false;
  gotEnd = false;
  ...
  async sanityCheck({ error }: { error?: string } = {}) {
    ...
    if (this.chatCompletions.some((c) => c.usage)) {
      const totalUsage: GigaChat.CompletionUsage = {
        completion_tokens: 0,
        prompt_tokens: 0,
        total_tokens: 0,
      };
...
class StreamingRunnerListener {
  readonly eventChunks: GigaChat.Chat.ChatCompletionChunk[] = [];
  readonly eventContents: [string, string][] = [];
  readonly eventMessages: ChatCompletionMessageParam[] = [];
  readonly eventChatCompletions: GigaChat.Chat.ChatCompletion[] = [];
  readonly eventFunctionCalls: GigaChat.Chat.ChatCompletionMessage.FunctionCall[] = [];
...
function _typeTests() {
  ...
  gigachat.beta.chat.completions.runTools({
    messages: [
...
describe('resource completions', () => {
  describe('runTools with stream: false', () => {
    test('successful flow', async () => {
      ...
      const gigachat = new GigaChat({ apiKey: 'something1234', baseURL: 'http://127.0.0.1:4010', fetch });
  ...
  describe('runTools with stream: true', () => {
    test('successful flow', async () => {
      ...
      await Promise.all([
        handleRequest(async function* (request): AsyncIterable<GigaChat.Chat.ChatCompletionChunk> {
          expect(request.messages).toEqual([{ role: 'user', content: 'tell me what the weather is like' }]);
          yield {
            id: '1',
            choices: [
    ...
    test('flow with abort', async () => {
      ...
      await handleRequest(async function* (request): AsyncIterable<GigaChat.Chat.ChatCompletionChunk> {
        expect(request.messages).toEqual([{ role: 'user', content: 'tell me what the weather is like' }]);
        yield {
          id: '1',
          choices: [
  ...
  describe('stream', () => {
    ...
    test('handles network errors', async () => {
      ...
      const gigachat = new GigaChat({ apiKey: '...', fetch });
