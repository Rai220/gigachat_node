 ...
 */
export class GigaChat extends Core.APIClient {
  apiKey: string;
  ...
  constructor({
    ...
    if (apiKey === undefined) {
      throw new Errors.GigaChatError(
        "The OPENAI_API_KEY environment variable is missing or empty; either provide it, or instantiate the GigaChat client with an apiKey option, like new GigaChat({ apiKey: 'My API Key' }).",
      );
    }
    ...
    if (!options.dangerouslyAllowBrowser && Core.isRunningInBrowser()) {
      throw new Errors.GigaChatError(
        "It looks like you're running in a browser-like environment.\n\nThis is disabled by default, as it risks exposing your secret API credentials to attackers.\nIf you understand the risks and have appropriate mitigations in place,\nyou can set the `dangerouslyAllowBrowser` option to `true`, e.g.,\n\nnew GigaChat({ apiKey, dangerouslyAllowBrowser: true });\n\nhttps://help.gigachat.com/en/articles/5112595-best-practices-for-api-key-safety\n",
      );
    }
  ...
  protected override defaultHeaders(opts: Core.FinalRequestOptions): Core.Headers {
    return {
      ...
      'GigaChat-Organization': this.organization,
      'GigaChat-Project': this.project,
  ...
  static GigaChat = this;
  static DEFAULT_TIMEOUT = 600000; // 10 minutes

  static GigaChatError = Errors.GigaChatError;
...
GigaChat.Completions = Completions;
GigaChat.Chat = Chat;
GigaChat.Embeddings = Embeddings;
GigaChat.Files = Files;
GigaChat.FileObjectsPage = FileObjectsPage;
GigaChat.Images = Images;
GigaChat.Audio = Audio;
GigaChat.Moderations = Moderations;
GigaChat.Models = Models;
GigaChat.ModelsPage = ModelsPage;
GigaChat.FineTuning = FineTuning;
GigaChat.Beta = Beta;
GigaChat.Batches = Batches;
GigaChat.BatchesPage = BatchesPage;
GigaChat.Uploads = UploadsAPIUploads;

export declare namespace GigaChat {
...
/** API Client for interfacing with the Azure GigaChat API. */
export interface AzureClientOptions extends ClientOptions {
  ...
  /**
   * Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.gigachat.com/`
   */
...
/** API Client for interfacing with the Azure GigaChat API. */
export class AzureGigaChat extends GigaChat {
   ...
   * @param {string | undefined} [opts.endpoint=process.env['AZURE_OPENAI_ENDPOINT'] ?? undefined] - Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.gigachat.com/`
   * @param {string | undefined} [opts.apiKey=process.env['AZURE_OPENAI_API_KEY'] ?? undefined]
   * @param {string | undefined} opts.deployment - A model deployment, if given, sets the base client URL to include `/deployments/{deployment}`.
   * @param {string | null | undefined} [opts.organization=process.env['OPENAI_ORG_ID'] ?? null]
   * @param {string} [opts.baseURL=process.env['OPENAI_BASE_URL']] - Sets the base URL for the API, e.g. `https://example-resource.azure.gigachat.com/gigachat/`.
   * @param {number} [opts.timeout=10 minutes] - The maximum amount of time (in milliseconds) the client will wait for a response before timing out.
   * @param {number} [opts.httpAgent] - An HTTP agent used to manage HTTP(s) connections.
   * @param {Core.Fetch} [opts.fetch] - Specify a custom `fetch` function implementation.
   * @param {number} [opts.maxRetries=2] - The maximum number of times the client will retry a request.
  ...
  constructor({
    ...
    if (!apiVersion) {
      throw new Errors.GigaChatError(
        "The OPENAI_API_VERSION environment variable is missing or empty; either provide it, or instantiate the AzureGigaChat client with an apiVersion option, like new AzureGigaChat({ apiVersion: 'My API Version' }).",
      );
    }
    ...
    if (!baseURL) {
      ...
      baseURL = `${endpoint}/gigachat`;
    } else {
      if (endpoint) {
        throw new Errors.GigaChatError('baseURL and endpoint are mutually exclusive');
      }
    }
  ...
  protected override async prepareOptions(opts: Core.FinalRequestOptions<unknown>): Promise<void> {
    ...
    if (token) {
      ...
      throw new Errors.GigaChatError('Unable to handle auth');
    }
    return super.prepareOptions(opts);
  }
}
