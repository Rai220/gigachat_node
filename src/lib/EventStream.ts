import { APIUserAbortError, GigaChatError } from '../error';
...
export class EventStream<EventTypes extends BaseEvents> {
  ...
  #rejectConnectedPromise: (error: GigaChatError) => void = () => {};

  #endPromise: Promise<void>;
  #resolveEndPromise: () => void = () => {};
  #rejectEndPromise: (error: GigaChatError) => void = () => {};
  ...
  #handleError(this: EventStream<EventTypes>, error: unknown) {
    ...
    if (error instanceof Error) {
      const openAIError: GigaChatError = new GigaChatError(error.message);
      // @ts-ignore
      openAIError.cause = error;
      return this._emit('error', openAIError);
    }
    return this._emit('error', new GigaChatError(String(error)));
  }
  ...
  _emit<Event extends keyof EventTypes>(
    ...
    if (event === 'error') {
      ...
      const error = args[0] as GigaChatError;
      if (!this.#catchingPromiseCreated && !listeners?.length) {
...
export interface BaseEvents {
  connect: () => void;
  error: (error: GigaChatError) => void;
