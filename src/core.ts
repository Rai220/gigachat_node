...
export class APIPromise<T> extends Promise<WithRequestID<T>> {
   ...
   * or add one of these imports before your first `import … from 'gigachat'`:
   * - `import 'gigachat/shims/node'` (if you're running on Node)
   * - `import 'gigachat/shims/web'` (otherwise)
   */
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new GigaChatError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new GigaChatError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null) throw new GigaChatError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new GigaChatError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`GigaChat:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new GigaChatError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
